...
void log_msg(const char *format, ...)
{
    ...
    vfprintf(TFS_DATA->logfile, format, ap);
}
...
void log_fuse_context(struct fuse_context *context)
{
    ...
    log_struct(((struct tfs_state *)context->private_data), logfile, %08x, );
    log_struct(((struct tfs_state *)context->private_data), rootdir, %s, );
